...
def create_url(list, index):
    base_url = 'https://zozo.jp/ranking/category/{}/all-sales-women.html'.format(list[index])
    return base_url
...
list = ['tops', 'pants', 'jacket-outerwear', 'skirt', 'onepiece']
index = 0
while index < len(list):
    f = open('./clothes/women_{}.txt'.format(list[index]), "w", encoding = "utf_8")
    do_func(list, index)
    print("Next category")
    index += 1
